...
use crate::parser::DocParserOptions;
  ...
  ( $name:ident, $source:expr, $private:expr; $block:expr; $diagnostics:expr ) => {
    ...
    async fn $name() {
      ...
      let parser = DocParser::new(
        ...
        DocParserOptions { private, diagnostics: true }
      ).unwrap();
      let entries = parser
        .parse(&specifier)
        .unwrap();
...
export function foo([e,,f, ...g]: number[], { c, d: asdf, i = "asdf", ...rest}, ops: AssignOpts = {}): void {
  ...
  }], vec!["file:///test.ts:2:11 PrivateTypeRef"]);
...
export interface Reader extends Foo, Bar {
    ...
    "file:///test.ts:2:11 PrivateTypeRef",
    "file:///test.ts:4:11 PrivateTypeRef"
  ]);
