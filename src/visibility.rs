  ...
  pub fn build(
    ...
    while let Some(original_id) = pending_symbol_ids.pop() {
      ...
      let decl_deps = symbol
        ...
        .flat_map(|decl| decl.deps(ResolveDepsMode::TypesOnly).into_iter().map(move |dep| (decl, dep)))
          ...
          symbol
            ...
            .flat_map(|symbol| {
              symbol
                ...
                .flat_map(move |decl| {
                  decl.deps(ResolveDepsMode::TypesOnly).into_iter().map(move |dep| (symbol, decl, dep))
                })
            }),
        );
