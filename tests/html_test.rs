...
async fn symbol_group() {
    ...
    let ctx = GenerateCtx {
        ...
        main_entrypoint: Some(ModuleSpecifier::from_file_path(multiple_dir.join("a.ts")).unwrap()),
...

    {
        for (specifier, doc_nodes) in &doc_nodes_by_url {
            ...
            let partitions_for_nodes = get_partitions_for_file(doc_nodes, &short_path);
