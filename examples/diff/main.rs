...
async fn run() -> anyhow::Result<()> {
  let matches = App::new("ddoc")
    .arg(Arg::with_name("source_files").number_of_values(2).required(true).multiple(true))
    .arg(
  ...
  for source_file in source_files {
    ...
    diff.push(doc_nodes_by_url.into_iter().next().unwrap().1);
  }

  
  dbg!(deno_doc::diff::diff(&diff[0], &diff[1]));
